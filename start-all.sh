#!/bin/bash
# AeroCommerce Microservices - Start All Services
...
echo "🚀 Starting AeroCommerce Microservices..."
echo ""

# Start UserService (port 5001)
echo "👤 Starting UserService on http://localhost:5001 ..."
dotnet run --project "$SCRIPT_DIR/src/AeroCommerce.UserService" &
USER_PID=$!
...
dotnet run --project "$SCRIPT_DIR/src/AeroCommerce.PostService" &
POST_PID=$!
...
dotnet run --project "$SCRIPT_DIR/src/AeroCommerce.ProductService" &
PRODUCT_PID=$!
...
dotnet run --project "$SCRIPT_DIR/src/AeroCommerce.CartService" &
CART_PID=$!
...
dotnet run --project "$SCRIPT_DIR/src/AeroCommerce.OrderService" &
ORDER_PID=$!
...
dotnet run --project "$SCRIPT_DIR/src/AeroCommerce.ApiGateway" &
GW_PID=$!
...
# Wait and cleanup
trap "echo ''; echo 'Stopping all services...'; kill $USER_PID $POST_PID $PRODUCT_PID $CART_PID $ORDER_PID $GW_PID 2>/dev/null; pkill -f 'AeroCommerce' 2>/dev/null; pkill -f 'dotnet' 2>/dev/null; exit" INT TERM
wait
